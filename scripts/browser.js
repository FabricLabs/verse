...
async function main (input) {
  window.addEventListener('load', () => {
    ...
    // Rover
    let vehicleGeometry = new THREE.SphereGeometry(0.5, 32, 32);
    let vehicleMesh = new THREE.MeshBasicMaterial({ color: 0x2200CC });
    let vehicleGlow = new THREE.PointLight(0xffffee, 0.5);
    let vehicle = new THREE.Mesh(vehicleGeometry, vehicleMesh);

    scene.add(vehicle);
    scene.add(vehicleGlow);
    ...
    function createFireballMesh (type = 1) {
      ...
      const fireballMesh = new THREE.MeshBasicMaterial({ color: 0xFF0000 });
      const fireball = new THREE.Mesh(fireballGeometry, fireballMesh);
      return fireball;
    }
